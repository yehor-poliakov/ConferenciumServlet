        ...
        <div>
                ...
                <c:if test="${sessionScope.role == 'PARTICIPANT'}">
                    <c:if test="${!conference.registered}">
                        <form action="/conference/${conference.id}/signup" method="post">
                            <button class="btn btn-primary" type="submit"><fmt:message key="registerButton" bundle="${bundle}"/></button>
                        </form>
                    </c:if>
                    <c:if test="${conference.registered}">
                        <form action="/conference/${conference.id}/signout" method="post">
                            <button class="btn btn-danger" type="submit"><fmt:message key="unregisterButton" bundle="${bundle}"/></button>
                        </form>
                    </c:if>
                </c:if>
                <table class="table">
                        ...
                        <c:forEach items="${presentations}" var="presentation">
                            <tr style="transform: rotate(0);">
                                    ...
                                    <td>
                                        <c:if test="${sessionScope.role == 'SPEAKER'}">
                                            <form action="presentation/${presentation.id}/suggest-speaker" method="post">
                                                <button class="btn-primary" type="submit"><fmt:message key="speakerApplyInscription" bundle="${bundle}"/></button>
                                            </form>
                                        </c:if>
                                    </td>
                                </c:if>
                                ...
                                <c:if test="${sessionScope.role == 'SPEAKER' || sessionScope.role == 'MODERATOR'}">
                                    ...
                                    <td><input type="checkbox" disabled ${presentation.presentationApproved ? 'checked' : ''}/></td>
                                </c:if>
            ...
            <c:if test="${sessionScope.role == 'MODERATOR'}">
                <form action="presentation/create/${conference.id}">
                    <button class="btn btn-success" type="submit"><fmt:message key="newPresentationLink" bundle="${bundle}"/></button>
                </form>
