                                            ...
                                            <div class="col-auto ml-3 pt-1">
                                                <div class="form-check align-items-center h-100">
                                                    <input class="form-check-input align-text-bottom mt-9" type="checkbox"
                                                        name="showPast" ${filters.showPast ? 'checked' : ''} id="showPast" onChange="this.form.submit()"/>
                                                ...
                                                <div class="input-group mb-2 align-items-center">
                                                    ...
                                                    <select name="orderby" name="orderby" id="orderby"
                                                            ...
                                                            <c:forEach items="${orderbyOptions}" var="orderbyOption">
                                                                <option ${filters.orderby.toString() == orderbyOption ? 'selected' : ''} value="${orderbyOption}"><fmt:message key="${orderbyOption}Form" bundle="${bundle}"/></option>
                                                            </c:forEach>
                                                    </select>
                                                </div>
                                            </div>
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <ul class="list-group list-group-flush">
                                                        <li class="list-group-item">
                                                            ...
                                                            :&nbsp;
                                                            <fmt:parseDate value="${conference.date}" pattern="yyyy-MM-dd" var="parsedDate" type="date" parseLocale="en_GB" />
                                                            <fmt:formatDate value="${parsedDate}" type="date" pattern="dd-MMM-yyyy"/>
                                                        </li>
                                                        <li class="list-group-item">
                                                            <fmt:message key="PresentationsNumberInscription" bundle="${bundle}"/>
                                                            :&nbsp;
                                                            ${conference.presentationsCount}
                                                        </li>
                                                        <li class="list-group-item">
                                                            <fmt:message key="ParticipantsNumberInscription" bundle="${bundle}"/>
                                                            :&nbsp;
                ...
                <nav style="margin-top:40px;">
                    <ul class="pagination justify-content-center">
                        <li class="${page.first ? 'page-item disabled' : 'page-item'}">
                            <a class="page-link" href="conferences?pageNumber=${page.number-1}&pageSize=${page.size}&showPast=${filters.showPast ? 'on' : 'off'}&showFuture=${filters.showFuture ? 'on' : 'off'}&orderby=${filters.orderby}">Previous</a>
                        </li>
                        <c:forEach begin="1" end="${page.totalPages}" var="i">
                            <li class="${page.number == i ? 'page-item active' : 'page-item'}">
                                <a class="page-link" href="conferences?pageNumber=${i}&pageSize=${page.size}&showPast=${filters.showPast ? 'on' : 'off'}&showFuture=${filters.showFuture ? 'on' : 'off'}&orderby=${filters.orderby}">${i}</a>
                            </li>
                        </c:forEach>
                        <li class="${page.last ? 'page-item disabled' : 'page-item'}">
                            <a class="page-link" href="conferences?pageNumber=${page.number+1}&pageSize=${page.size}&showPast=${filters.showPast ? 'on' : 'off'}&showFuture=${filters.showFuture ? 'on' : 'off'}&orderby=${filters.orderby}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
